...
async function run({ api, event, args, getLang, prefix }) {
    const { type, messageReply, senderID, threadID } = event;
    const attachments = type == "message_reply" ? messageReply.attachments : event.attachments;
    let msg = (type == "message_reply" && messageReply.body ? messageReply.body : event.body.slice(prefix.length + config.name.length + 1)) || "";
    ...
    api.sendMessage(resultMsg, threadID, event.messageID);
}
